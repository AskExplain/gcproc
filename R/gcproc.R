...
gcproc <- function(data_list,
  ...
  if (config$identify){
    ...
    config$i_dim <- dim(data_list[[1]])[1]
...
update_set <- function(x,
  ...
  if (config$identify){
    ...
    main.parameters$alpha <- diag(dim(x)[1])
    main.parameters$beta <- t((pinv(main.code$code))%*%t((main.code$code))%*%x)
    ...
    if (method == "gcproc"){
      main.code$code <- (main.code$encode)%*%pinv(main.parameters$beta)
    }
  ...
  if (config$diagonal & config$i_dim == config$j_dim){
    
    main.code$code <- diag(diag(main.code$code))
    id_latent <- order(diag(main.code$code),decreasing = T)
    
    main.code$code <- diag(diag(main.code$code)[id_latent])
    main.parameters$alpha <- main.parameters$alpha[id_latent,]
    main.parameters$beta <- main.parameters$beta[,id_latent]
