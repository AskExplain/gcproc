initialise.gcproc <- function(x,y,k_dim=70,j_dim=70,init="svd",verbose=F){
  ...
  cov_x <- corpcor::cov.shrink((x),lambda = 0.95,lambda.var = 0.95,verbose = verbose)
  u.beta.svd <- irlba::irlba(
    cov_x,j_dim,tol=1e-10,maxit = 10000)
  rm(cov_x)

  cov_y <- corpcor::cov.shrink((y),lambda = 0.95,lambda.var = 0.95,verbose = verbose)
  ...
  cov_tx <- corpcor::cov.shrink(Matrix::t(x),lambda = 0.95,lambda.var = 0.95,verbose = verbose)
  alpha.L.J.svd <- irlba::irlba(
    cov_tx,k_dim,tol=1e-10,maxit = 10000)
  rm(cov_tx)

  cov_ty <- corpcor::cov.shrink(Matrix::t(y),lambda = 0.95,lambda.var = 0.95,verbose = verbose)
