...
#' @param main.code Code parameters from gcproc
        ...
        for (i in 1:length(data_list)){
          ...
          if (!is.null(recover$design.list[[i]])){
            ...
            if ("decode" %in% method){
              ...
              pred <- t(main.parameters[[i]]$alpha)%*%main.code$main.code%*%t(main.parameters[[i]]$beta)
            ...
            if ("matrix.projection" %in% method){
              ...
              if (is.null(recover$enmain.coded_covariate)){
                recover$enmain.coded_covariate <- lapply(c(1:length(data_list)),function(X){
                  # Predict using enmain.code space
              ...
              demain.coded_covariate <- cbind(1,scale(Reduce('+',lapply(c(1:length(recover$enmain.coded_covariate))[-i],function(X){
                t(main.parameters[[i]]$alpha)%*%recover$enmain.coded_covariate[[X]]
              }))))

              samples_with_missing_points <- which((rowSums(recover$design.list[[i]])>0)==T)
              covariate_predictors <-  demain.coded_covariate[-samples_with_missing_points,]
              test_predictors <- demain.coded_covariate[samples_with_missing_points,]
    ...
    if ("classification" %in% task){
      ...
      if (label.projection){
        ...
        for (j in which(recover$design.list==0)){
          ...
          label.demain.coded_covariate <- scale(Reduce('+',lapply(c(1:length(recover$enmain.coded_covariate))[-i],function(X){
            t(main.parameters[[j]]$alpha)%*%recover$enmain.coded_covariate[[X]]
          })))

          for (i in which(recover$design.list==1)){

            unlabel.demain.coded_covariate <- scale(Reduce('+',lapply(c(1:length(recover$enmain.coded_covariate))[-j],function(X){
              t(main.parameters[[i]]$alpha)%*%recover$enmain.coded_covariate[[X]]
            })))

            labels <- recover$labels

            recover$predict.list[[j]][[i]] <- apply((unlabel.demain.coded_covariate)%*%t(label.demain.coded_covariate),1,function(X){names(sort(table(labels[order(X,decreasing = T)[1]]))[1])})
