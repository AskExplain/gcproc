...
Three models are run: gcproc, glmnet and lm - respectively they are Generalised Canonical Procrustes, a Regularised Generalised Linear Model, and a Standard Linear Model. On smaller datasets, it is expected for a linear model to perform well, whereas on larger datasets with more features it is expected a regularised method such as glmnet to perform well.  The root mean squared error is reported, along with the pearson correlation.
