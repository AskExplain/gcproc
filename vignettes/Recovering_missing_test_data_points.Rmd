...
for (list_id in c(1:length(predict.list))){
  ...
  for (seed in c(1:100)){
    ...
    recover$x <- recover$y <- array(0,dim=dim(data___x))
    recover$x[-train_id,y_predict.id_col] <- recover$y[-train_id,y_predict.id_col] <- 1
    
    # Initialise missing values given by recovery design matrix with median feature value
    x.x.check <- as.matrix(data___x)
    x.x.check <- x.x.check*(1-recover$y) + do.call('cbind',lapply(c(1:dim(x.x.check*(1-recover$y))[2]),function(X){(recover$y[,X])*median(x.x.check[which((1-recover$y)[,X]==1),X])}))
    ...
    gcproc.recover <- gcproc.model$recover$predict.y
    
    y_hat.plot <- gcproc.recover[which(recover$y==1,arr.ind = T)]
    data___x.plot <- data___x[which((recover$y)==1,arr.ind = T)]
...
for (list_id in c(1:length(predict.list))){
  ...
  for (seed in 1:100){
    ...
    recover <- gcproc::extract_recovery_framework(F)
    recover$y <- recover$x <- (data___x.check$where)
    ...
    gcproc.recover <- gcproc.model$recover$predict.x
